...
class EpisodeArtifactsTest extends TestCase
{
    use TestHelper;

    public function testList(): void
    {
        $fixture = $this->loadFixture('list_envelope_list_episode_artifact');
        $client = $this->createMockClient([['status' => 200, 'body' => $fixture]]);
        $result = $client->episodeArtifacts()->list(episodeId: 'test_value', limit: 1, startingAfter: 'test_value');
        $this->assertInstanceOf(\Rafflesia\PaginatedResponse::class, $result);
        $request = $this->getLastRequest();
        $this->assertSame('GET', $request->getMethod());
        $this->assertStringEndsWith('v1/episode_artifacts', $request->getUri()->getPath());
        parse_str($request->getUri()->getQuery(), $query);
        $this->assertSame('test_value', $query['episode_id']);
    ...
    public function testPaginationBoundary(): void
    {
        ...
        $client = $this->createMockClient([['status' => 200, 'body' => $fixture]]);
        $result = $client->episodeArtifacts()->list();
        $this->assertInstanceOf(\Rafflesia\PaginatedResponse::class, $result);
